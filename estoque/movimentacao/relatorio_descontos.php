...
<?php
if ($filial_origem_pesq == "JAGUARE" and $filial_armazem_pesq == "LINHARES")
{$fornecedor_filial = "900";}
elseif ($filial_origem_pesq == "SAO_MATEUS" and $filial_armazem_pesq == "LINHARES")
{$fornecedor_filial = "3137";}
else
{$fornecedor_filial = "-";}


// ====== BUSCA PESSOA ===================================================================================
$busca_pessoa = mysqli_query ($conexao, "SELECT * FROM cadastro_pessoa WHERE codigo='$fornecedor_filial' AND estado_registro!='EXCLUIDO'");
...
$filial_quant_liq = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(quantidade) FROM estoque WHERE estado_registro!='EXCLUIDO' AND movimentacao='ENTRADA' AND situacao_romaneio='FECHADO' AND $mysql_filtro_data AND fornecedor='$fornecedor_filial' AND $mysql_filial_armazem AND $mysql_filial_origem AND cod_produto='$cod_produto_busca'"));

$filial_quant_liq_2 = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(quantidade) FROM estoque WHERE estado_registro!='EXCLUIDO' AND movimentacao='ENTRADA' AND situacao_romaneio='FECHADO' AND $mysql_filtro_data AND fornecedor!='$fornecedor_filial' AND $mysql_filial_armazem AND $mysql_filial_origem AND cod_produto='$cod_produto_busca'"));

$filial_quant_liq_3 = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(quantidade) FROM estoque WHERE estado_registro!='EXCLUIDO' AND movimentacao='ENTRADA' AND situacao_romaneio='FECHADO' AND $mysql_filtro_data AND filial='$filial_origem_pesq' AND cod_produto='$cod_produto_busca'"));

$filial_quant_previsto = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(quant_quebra_previsto) FROM estoque WHERE estado_registro!='EXCLUIDO' AND movimentacao='ENTRADA' AND situacao_romaneio='FECHADO' AND $mysql_filtro_data AND fornecedor='$fornecedor_filial' AND $mysql_filial_armazem AND $mysql_filial_origem AND cod_produto='$cod_produto_busca'"));

$filial_quant_previsto_2 = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(quant_quebra_previsto) FROM estoque WHERE estado_registro!='EXCLUIDO' AND movimentacao='ENTRADA' AND situacao_romaneio='FECHADO' AND $mysql_filtro_data AND fornecedor!='$fornecedor_filial' AND $mysql_filial_armazem AND $mysql_filial_origem AND cod_produto='$cod_produto_busca'"));

$filial_quant_realizado = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(quant_quebra_realizado) FROM estoque WHERE estado_registro!='EXCLUIDO' AND movimentacao='ENTRADA' AND situacao_romaneio='FECHADO' AND $mysql_filtro_data AND fornecedor!='$fornecedor_filial' AND filial='$filial_origem_pesq' AND cod_produto='$cod_produto_busca'"));
if ($cod_produto_busca == 2 or $cod_produto_busca == 10)
{$filial_quant_realizado_x = $filial_quant_realizado[0]*60;}
else
{$filial_quant_realizado_x = $filial_quant_realizado[0];}

$filial_quant_realizado_2 = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(quant_quebra_realizado) FROM estoque WHERE estado_registro!='EXCLUIDO' AND movimentacao='ENTRADA' AND situacao_romaneio='FECHADO' AND $mysql_filtro_data AND fornecedor!='$fornecedor_filial' AND $mysql_filial_armazem AND $mysql_filial_origem AND cod_produto='$cod_produto_busca'"));
if ($cod_produto_busca == 2 or $cod_produto_busca == 10)
{$filial_quant_realizado_x2 = $filial_quant_realizado_2[0]*60;}
else
{$filial_quant_realizado_x2 = $filial_quant_realizado_2[0];}


$filial_quant_liq_print = number_format($filial_quant_liq[0],0,",",".");
$filial_quant_liq_2_print = number_format($filial_quant_liq_2[0],0,",",".");
$filial_quant_liq_3_print = number_format($filial_quant_liq_3[0],0,",",".");
$filial_quant_previsto_print = number_format($filial_quant_previsto[0],0,",",".");
$filial_quant_previsto_2_print = number_format($filial_quant_previsto_2[0],0,",",".");
$filial_quant_realizado_print = number_format($filial_quant_realizado_x,0,",",".");
$filial_quant_realizado_2_print = number_format($filial_quant_realizado_x2,0,",",".");

if ($filial_quant_liq_print != 0 and $fornecedor_filial != "-")
{
echo "
<div style='width:1200px; height:26px; border:0px solid #000; margin-left:20px; margin-top:10px'>
	...
	<div style='width:360px; height:24px; border:1px solid #009900; color:#003466; font-size:12px; float:left; margin-left:10px'>
		<div style='margin-left:10px; margin-top:6px'>Transfer&ecirc;ncias entre filiais: $filial_origem_pesq X $filial_armazem_pesq</div>
	</div>
	...
	<div style='width:250px; height:24px; border:1px solid #009900; color:#003466; font-size:12px; float:left; margin-left:10px'>
		<div style='margin-left:10px; margin-top:6px'></div>
	</div>
</div>
...
<div style='width:1200px; height:26px; border:0px solid #000; margin-left:20px; margin-top:10px'>
	...
	<div style='width:360px; height:24px; border:1px solid #009900; color:#003466; font-size:12px; float:left; margin-left:10px'>
		<div style='margin-left:10px; margin-top:6px'>Romaneios Fornecedores de $filial_origem_pesq:</div>
	</div>
	
	<div style='width:250px; height:24px; border:1px solid #009900; color:#003466; font-size:12px; float:left; margin-left:10px'>
		<div style='margin-left:10px; margin-top:6px'>Total: $filial_quant_liq_2_print Kg</div>
	</div>
	
	<div style='width:250px; height:24px; border:1px solid #009900; color:#003466; font-size:12px; float:left; margin-left:10px'>
		<div style='margin-left:10px; margin-top:6px'>Desconto Previsto: $filial_quant_previsto_2_print Kg</div>
	</div>
	
	<div style='width:250px; height:24px; border:1px solid #009900; color:#003466; font-size:12px; float:left; margin-left:10px'>
		<div style='margin-left:10px; margin-top:6px'>Desconto Realizado: $filial_quant_realizado_2_print Kg</div>
	</div>
</div>
...
<div style='width:1200px; height:26px; border:0px solid #000; margin-left:20px; margin-top:10px'>
	...
	<div style='width:360px; height:24px; border:1px solid #009900; color:#003466; font-size:12px; float:left; margin-left:10px'>
		<div style='margin-left:10px; margin-top:6px'>Romaneios internos de $filial_origem_pesq:</div>
	</div>
	
	<div style='width:250px; height:24px; border:1px solid #009900; color:#003466; font-size:12px; float:left; margin-left:10px'>
		<div style='margin-left:10px; margin-top:6px'>Total: $filial_quant_liq_3_print Kg</div>
	</div>
	...
	<div style='width:250px; height:24px; border:1px solid #009900; color:#003466; font-size:12px; float:left; margin-left:10px'>
		<div style='margin-left:10px; margin-top:6px'>Desconto Realizado: $filial_quant_realizado_print Kg</div>
	</div>
</div>
