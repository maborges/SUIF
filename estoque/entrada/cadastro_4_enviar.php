...
$data_inicial_busca_br = $_POST['data_inicial_busca_br'] ?? '';
$data_final_busca_br = $_POST['data_final_busca_br'] ?? '';

$cod_lote_form = $_POST["cod_lote_form"] ?? '';
$data_inicial_br = $_POST["data_inicial_br"] ?? '';
$data_final_br = $_POST["data_final_br"] ?? '';
$cod_produto_buscar = $_POST["cod_produto_buscar"] ?? '';
$fornecedor_buscar = $_POST["fornecedor_buscar"] ?? '';
$numero_romaneio_buscar = $_POST["numero_romaneio_buscar"] ?? '';
$situacao_romaneio_buscar = $_POST["situacao_romaneio_buscar"] ?? '';
$forma_pesagem_buscar = $_POST["forma_pesagem_buscar"] ?? '';
...
$usuario_finalizacao = $nome_usuario_print;
$hora_finalizacao = date('G:i:s', time());
$data_finalizacao = date('Y-m-d', time());
// ================================================================================================================
...
// ====== BUSCA NUMERO DE ROMANEIO (DUPLICIDADE) ==============================================================
...
$achou_romaneio_duplicidade = mysqli_num_rows($busca_num_romaneio);
// ============================================================================================================


// ====== BUSCA ROMANEIO MANUAL - COMPRAS ======================================================================
$busca_rmc = mysqli_query($conexao, "SELECT * FROM compras WHERE estado_registro!='EXCLUIDO' AND num_romaneio_manual='$romaneio_manual_form'");
...
$busca_rme = mysqli_query($conexao, "SELECT * FROM estoque WHERE estado_registro!='EXCLUIDO' AND (numero_romaneio='$romaneio_manual_form' OR num_romaneio_manual='$romaneio_manual_form')");
...
if ($filial_config[9] == "S") {
	$quantidade = ($peso_inicial - $peso_final - $desconto_sacaria - $desconto);
} else {
...
if ($botao == "NOVO_ROMANEIO") {
	if ($fornecedor_form == "" or $linhas_fornecedor == 0) {
		...
		if ($config[32] == "S" and ($fornecedor == "900" or $fornecedor == "3137" or $fornecedor == "6856")) {
			$inserir = mysqli_query($conexao, "INSERT INTO estoque (codigo, numero_romaneio, fornecedor, data, produto, tipo, peso_inicial, 
			peso_final, desconto_sacaria, desconto, quantidade, unidade, tipo_sacaria, movimentacao, situacao, situacao_romaneio, placa_veiculo, 
			motorista, observacao, usuario_cadastro, hora_cadastro, data_cadastro, filial, estado_registro, quantidade_sacaria, motorista_cpf, 
			num_romaneio_manual, filial_origem, quant_volume_sacas, cod_produto, cod_unidade, cod_tipo, fornecedor_print, usuario_finalizacao, 
			hora_finalizacao, data_finalizacao, transferencia_filiais, transferencia_numero) VALUES (NULL, '$numero_romaneio', 
			'$fornecedor_form', '$data_romaneio', '$produto_print', '$tipo_print', '$peso_inicial', '$peso_final', '$desconto_sacaria', 
			'$desconto', '$quantidade', '$un_aux', '$cod_sacaria_form', '$movimentacao', '$situacao', '$situacao_romaneio', '$placa_veiculo_form', 
			'$motorista_form', '$obs_form', '$usuario_cadastro', '$hora_cadastro', '$data_cadastro', '$filial', '$estado_registro', '$quant_sacaria', 
			'$motorista_cpf_form', '$romaneio_manual_form', '$filial_origem_form', '$quant_volume', '$cod_produto_form', '$cod_un_aux', 
			'$cod_tipo_produto_form', '$fornecedor_print', '$usuario_finalizacao', '$hora_finalizacao', '$data_finalizacao', 'SIM', '$numero_romaneio')");
		} else {
			$inserir = mysqli_query($conexao, "INSERT INTO estoque (codigo, numero_romaneio, fornecedor, data, produto, tipo, peso_inicial, 
				peso_final, desconto_sacaria, desconto, quantidade, unidade, tipo_sacaria, movimentacao, situacao, situacao_romaneio, placa_veiculo, 
				motorista, observacao, usuario_cadastro, hora_cadastro, data_cadastro, filial, estado_registro, quantidade_sacaria, motorista_cpf, 
				num_romaneio_manual, filial_origem, quant_volume_sacas, cod_produto, cod_unidade, cod_tipo, fornecedor_print, usuario_finalizacao, 
				hora_finalizacao, data_finalizacao) VALUES (NULL, '$numero_romaneio', '$fornecedor_form', '$data_romaneio', '$produto_print', 
				'$tipo_print', '$peso_inicial', '$peso_final', '$desconto_sacaria', '$desconto', '$quantidade', '$un_aux', '$cod_sacaria_form', 
				'$movimentacao', '$situacao', '$situacao_romaneio', '$placa_veiculo_form', '$motorista_form', '$obs_form', '$usuario_cadastro', 
				'$hora_cadastro', '$data_cadastro', '$filial', '$estado_registro', '$quant_sacaria', '$motorista_cpf_form', '$romaneio_manual_form', 
				'$filial_origem_form', '$quant_volume', '$cod_produto_form', '$cod_un_aux', '$cod_tipo_produto_form', '$fornecedor_print', 
				'$usuario_finalizacao', '$hora_finalizacao', '$data_finalizacao')");
		}

		// ====== ROMANEIO DE SAÍDA AUTOMATICO =======================================================================
		if ($config[32] == "S") {
			include('../../includes/romaneio_automatico.php');
		}
		// ===========================================================================================================
			...
			<div style="width:169px; height:50px; border:1px solid transparent; margin-top:10px; float:left">
				...
				<div style="width:167px; height:25px; float:left; border:1px solid transparent">
					<div style="width:153px; height:25px; border:1px solid #999; float:left; font-size:12px; text-align:center; background-color:#EEE">
						<div style="margin-top:4px; margin-left:5px; width:143px; height:16px; text-align:left; overflow:hidden"><?php echo "$tipo_print" ?></div>
					</div>
				</div>
			</div>
			<!-- ================================================================================================================ -->
...
";
			...
			<div style="width:682px; height:50px; border:1px solid transparent; margin-top:10px; float:left">
				...
				<div style="width:674px; height:25px; float:left; border:1px solid transparent">
					<div style="width:666px; height:25px; border:1px solid #999; float:left; font-size:12px; text-align:left; background-color:#EEE">
						<div style="margin-top:5px; margin-left:5px; width:656px; height:14px; overflow:hidden"><?php echo "$obs_form" ?></div>
					</div>
				</div>
			</div>
			<!-- ============================================================================================================== -->
	...
	<div id='centro' style='float:left; height:55px; width:200px; color:#00F; text-align:center; border:0px solid #000'>
		...
		<input type='hidden' name='numero_romaneio' value='$numero_romaneio'>
		<input type='hidden' name='data_inicial_buscar' value='$data_inicial_br'>
		<input type='hidden' name='data_final_buscar' value='$data_final_br'>
	...
	<div id='centro' style='float:left; height:55px; width:200px; color:#00F; text-align:center; border:0px solid #000'>
		...
		<button type='submit' class='botao_2' style='margin-left:10px; width:180px; color:#BBB'>Imprimir</button>
				...
				echo "
	<div id='centro' style='float:left; height:55px; width:200px; color:#00F; text-align:center; border:0px solid #000'>
		<form action='$servidor/$diretorio_servidor/estoque/entrada/entrada_relatorio_produto.php' method='post'>
	...
	<div id='centro' style='float:left; height:55px; width:200px; color:#00F; text-align:center; border:0px solid #000'>
	<form name='voltar' action='$servidor/$diretorio_servidor/estoque/entrada/cadastro_3_formulario.php' method='post'>
	...
	<input type='hidden' name='numero_romaneio_buscar' value='$numero_romaneio'>
	<input type='hidden' name='data_inicial_buscar' value='$data_inicial_br'>
	<input type='hidden' name='data_final_buscar' value='$data_final_br'>
	<input type='hidden' name='cod_produto_buscar' value='$cod_produto_buscar'>
	<input type='hidden' name='fornecedor_buscar' value='$fornecedor_buscar'>
