...
// ======= RECEBENDO POST =================================================================================
$botao = $_POST["botao"] ?? '';
$botaoBuscaCC = $_POST["botaoBuscaCC"] ?? '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($botaoBuscaCC == 'botaoBuscaCC') {
		$botaoBuscaCC = 'NOVO_CADASTRO';

		if (isset($_POST['idSankhyaCC_form']) && $_POST['idSankhyaCC_form'] <> "") {
			$idSankhya_pessoa = $_POST['idSankhya_pessoa'];
			$idSankhyaCC_form = $_POST['idSankhyaCC_form'];

			// Conta Corrente
			$ContaCorrenteSankhya = Sankhya::getContaCorrenteByParceiro($idSankhya_pessoa, $idSankhyaCC_form);
...
$idSankhyaCC_form = $_POST["idSankhyaCC_form"] ?? '';
$idSankhyaCC_edicao_form = $_POST["idSankhyaCC_edicao_form"] ?? '0';
...
$idSankhya_pessoa = $aux_pessoa[41];
...
} else if ($idSankhya_pessoa == '') {
	$erro = 1;
	$msg = "<div style='color:#FF0000'>É necessário que o código Sankhya esteja informado para criar um favorecido.</div>";
}
// ======================================================================================================
...
<script>
	//	document.getElementById("botaoBuscaCC").value = '';
...
<body onload="javascript:foco('ok');">
	...
	<div class="ct_fixo" style="height:560px">
				...
				<div>
					...
					<div class="pqa_caixa">
						...
						<div class="pqa_campo" style="margin-left:0px;">
							<input readonly type="number" name="idSankhya_pessoa" class="pqa_input" id="idSankhya_pessoa" maxlength="8" style="width:145px; background-color:#EEE" value="<?php echo $idSankhya_pessoa ?>" />
						</div>
					</div>
		...
		<div style='margin-top:6px; margin-left:5px; width:143px; height:16px; color:#003466; text-align:left; overflow:hidden'>$tipo_pessoa_print</div></div>";
			...
			<div style="width:100%; height:50px; border:1px solid transparent;  float:left">
				<div>
					<form name="frmSankhyaCC" action="
						<?php
						echo "$servidor/$diretorio_servidor";
						?>/cadastros/favorecidos/cadastro_2_formulario.php" method="post">

						<input type="hidden" name="botaoBuscaCC" id="botaoBuscaCC" value="" />
						<input type="hidden" name="botao" value="BANCO" />
						<input type="hidden" name="nome_form" value="<?php echo "$nome_form"; ?>" />
						<input type="hidden" name="idSankhya_pessoa" value="<?php echo "$idSankhya_pessoa"; ?>" />
						...
						<div class="pqa_caixa">
							...
							<div class="pqa_campo" style="margin-left:0px;">
								<input <?php
										if ($idSankhyaCC_edicao_form) {
											echo "readonly";
										}
										?> type="number" name="idSankhyaCC_form" class="pqa_input" id="idSankhyaCC_form" maxlength="8" onchange="LimpaFormsCC()" style="width:145px; 
								   <?php
									if ($idSankhyaCC_edicao_form) {
										echo "background-color:#EEE;";
									}
									?>" value="<?php echo $idSankhyaCC_form ?>">
							</div>
						</div>
			...
			<div style="width:169px; height:50px; border:1px solid transparent; margin-top:10px; float:left">
				...
				<div style="width:167px; height:25px; float:left; border:1px solid transparent">
					<form name="banco" action="<?php echo "$servidor/$diretorio_servidor"; ?>/cadastros/favorecidos/cadastro_2_formulario.php" method="post">
						...
						<input type="hidden" name="idSankhya_pessoa" value="<?php echo "$idSankhya_pessoa"; ?>" />
						<input type="hidden" name="fornecedor_form" value="<?php echo "$fornecedor_form"; ?>" />
						<input type="hidden" name="idSankhyaCC_form" value="<?php echo "$idSankhyaCC_form"; ?>" />
						<input type="hidden" name="agencia_form" value="<?php echo "$agencia_form"; ?>" />
						<input type="hidden" name="numero_conta_form" value="<?php echo "$numero_conta_form"; ?>" />
						<input type="hidden" name="tipo_conta_form" value="<?php echo "$tipo_conta_form"; ?>" />
						<input type="hidden" name="conta_conjunta_form" value="<?php echo "$conta_conjunta_form"; ?>" />
						...
						<select readonly name="banco_form" id="banco_form" class="form_select" onchange="document.banco.submit()" style="background-color:#EEE; width:154px">
							<option></option>
							<?php
							include("../../includes/conecta_bd.php");
			...
			<div style="width:169px; height:50px; border:1px solid transparent; margin-top:10px; float:left">
				<form action="<?php echo "$servidor/$diretorio_servidor"; ?>/cadastros/favorecidos/cadastro_3_enviar.php" method="post">
					...
					<div style="width:167px; height:25px; float:left; border:1px solid transparent">
						<?php
						if ($banco_form == "") {
							echo "<input type='text' name='agencia_form' id='agencia_form' class='form_input' maxlength='8' onBlur='alteraMaiusculo(this)' 
        onkeydown='if (getKey(event) == 13) return false;' disabled='disabled' style='background-color:#EEE; width:145px; text-align:left; padding-left:5px' value='$agencia_form' />";
						} else {
							echo "<input type='text' name='agencia_form' id='agencia_form' class='form_input' maxlength='8' onBlur='alteraMaiusculo(this)' 
        onkeydown='if (getKey(event) == 13) return false;' style='background-color:#EEE; width:145px; text-align:left; padding-left:5px' value='$agencia_form'>";
						}
						?>
					</div>
			</div>
			...
			<div style="width:169px; height:50px; border:1px solid transparent; margin-top:10px; float:left">
				...
				<div style="width:167px; height:25px; float:left; border:1px solid transparent">
					<?php
					if ($banco_form == "") {
						echo "<input type='text' name='numero_conta_form' id='numero_conta_form' class='form_input' maxlength='20' id='conta_bancaria' onBlur='alteraMaiusculo(this)' 
        onkeydown='if (getKey(event) == 13) return false;' disabled='disabled' style='background-color:#EEE; width:145px; text-align:left; padding-left:5px' value='$numero_conta_form' />";
					} else {
						echo "<input type='text' name='numero_conta_form' id='numero_conta_form' class='form_input' maxlength='20' id='conta_bancaria' onBlur='alteraMaiusculo(this)' 
        onkeydown='if (getKey(event) == 13) return false;' style='background-color:#EEE; width:145px; text-align:left; padding-left:5px' value='$numero_conta_form' />";
					}
					?>
			...
			<div style="width:169px; height:50px; border:1px solid transparent; margin-top:10px; float:left">
				...
				<div style="width:167px; height:25px; float:left; border:1px solid transparent">
					<select readonly name="tipo_conta_form" id="tipo_conta_form" class="form_select" onkeydown="if (getKey(event) == 13) return false;" style="background-color:#EEE; width:154px">
						<option></option>
						<?php
			...
			<div style="width:169px; height:50px; border:1px solid transparent; margin-top:10px; float:left">
				...
				<div style="width:167px; height:25px; float:left; border:1px solid transparent">
					<select readonly name="conta_conjunta_form" id="conta_conjunta_form" class="form_select" onkeydown="if (getKey(event) == 13) return false;" style="background-color:#EEE; width:154px">
						<option></option>
						<?php
			...
			<div style="width:169px; height:50px; border:1px solid transparent; margin-top:10px; float:left">
				...
				<div style="width:167px; height:25px; float:left; border:1px solid transparent">
					<select readonly name="tipo_chave_pix_form" id="tipo_chave_pix_form" class="form_select" onkeydown="if (getKey(event) == 13) return false;" style="background-color:#EEE; width:154px">
						<option></option>
						<?php
			...
			<div style="width:339px; height:50px; border:1px solid transparent; margin-top:10px; float:left">
				...
				<div style="width:334px; height:25px; float:left; border:1px solid transparent">
					<?php
					echo "<input readonly type='text' name='chave_pix_form' id='chave_pix_form' class='form_input' maxlength='50' 
        onkeydown='if (getKey(event) == 13) return false;' style='background-color:#EEE; width:315px; text-align:left; padding-left:5px' value='$chave_pix_form'>";
					?>
			...
			<div style="width:510px; height:50px; border:1px solid transparent; margin-top:10px; float:left">
				...
				<div style="width:500px; height:25px; float:left; border:1px solid transparent">
					<input type="text" name="obs_form" id="obs_form" class="form_input" maxlength="80" onkeydown="if (getKey(event) == 13) return false;" style="background-color:#EEE; width:486px; text-align:left; padding-left:5px" value="<?php echo "$obs_form"; ?>" />
				</div>
			</div>
			<!-- ================================================================================================================ -->
	...
	<script type="text/javascript">
		function LimpaFormsCC() {
			...
			document.getElementById("conta_conjunta_form").vakue = "";
		...
		function BuscaCC(action) {
			document.getElementById("botaoBuscaCC").value = 'botaoBuscaCC';
