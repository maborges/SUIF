...
<div id="centro" style="height:25px; width:1080px; border:0px solid #000; margin:auto">
	...
	<input type='hidden' name='fornecedor' value='$fornecedor' />
	<input type='image' src='$servidor/$diretorio_servidor/imagens/icones/informacao.png' border='0' title='Consultar Tal&atilde;o' style='float:left' />
	</form>
	...
	// ====== CONTROLE DE CONTRATO FUTURO  ==========================================================================================
		...
		<input type='image' src='$servidor/$diretorio_servidor/imagens/icones/informacao.png' border='0' title='Consultar Contrato Futuro' style='float:left' />
		</form>
	...
	// ====== CONTROLE DE CONTRATO TRATADO  ==========================================================================================
		...
		<input type='image' src='$servidor/$diretorio_servidor/imagens/icones/informacao.png' border='0' title='Consultar Contrato Tratado' style='float:left' />
		</form>
...
<div id="centro" style="height:30px; width:1080px; border:0px solid #000; margin:auto">
	...
	<div id='centro' style='height:30px; width:40px; border:0px solid #000; color:#FF0000; font-size:12px; float:left'>
	<img src='$servidor/$diretorio_servidor/imagens/icones/lembrete.jpg' style='border:0' /></div>
	<div id='centro' style='height:auto; width:950px; border:0px solid #000; color:#FF0000; font-size:12px; float:left'>
	<font style='float:left; color:#FF0000; margin-top:5px'><i><b>Lembrete para lembrar de nao esquecer.</b></i></font>
	</div>";
	*/
	?>
</div>
...
<div id="centro" style="height:35px; width:1080px; border:0px solid #000; margin:auto">
	...
	<div id="centro" style="height:36px; width:1080px; border:1px solid #999; color:#666; font-size:11px; border-radius:5px; float:left; margin-left:0px">
		...
		<input type='hidden' name='fornecedor' value='$fornecedor' />
		<input type='hidden' name='cod_produto' value='$cod_produto' />
		<input type='image' src='$servidor/$diretorio_servidor/imagens/botoes/nova_compra.png' border='0' title='Nova Compra' style='float:left' />
		</form>";}
		else
		{}
		?>
		...
		<div id="centro" style="width:120px; float:left; border:0px solid #999; text-align:left; margin-top:6px">
		<form action="<?php echo"$servidor/$diretorio_servidor/compras/ficha_produtor/entrada_seleciona.php"; ?>" method="post" />
		<input type='hidden' name='aux_cod_produtor' value='<?php echo"$cod_produtor"; ?>' />
        <input type='hidden' name='fornecedor' value='<?php echo"$fornecedor"; ?>' />
		<input type='hidden' name='cod_produto' value='<?php echo"$cod_produto"; ?>' />
		<input type="image" src="<?php echo"$servidor/$diretorio_servidor"; ?>/imagens/botoes/entrada.jpg" border="0" title="Nova Entrada" style="float:left" />
		</form>
		</div>	
		...
		<input type='hidden' name='fornecedor' value='$fornecedor' />
		<input type='hidden' name='cod_produto' value='$cod_produto' />
		<input type='image' src='$servidor/$diretorio_servidor/imagens/botoes/talao.jpg' border='0' title='al&atilde;o de Saldo' style='float:left' />
		</form>";}
		else
		{}
		?>		
...
if ($movimentacao == "COMPRA")
{
	...
	<input type='hidden' name='cod_tipo' value='$cod_tipo'>
	<input type='hidden' name='monstra_situacao' value='$monstra_situacao'>
	<input type='image' src='$servidor/$diretorio_servidor/imagens/icones/icone_baixar.png' border='0' /></form>	
	</td>";
	// ----------------------------------------------------------------------------------------------------
...
elseif ($movimentacao == "TRANSFERENCIA_SAIDA" or $movimentacao == "SAIDA")
{
	...
	echo "
	<td width='54px' align='center'>
	<form action='$servidor/$diretorio_servidor/compras/ficha_produtor/transferencia_visualizar.php' method='post'>
	<input type='hidden' name='numero_compra' value='$numero_compra'>
	<input type='hidden' name='pagina_mae' value='$pagina'>
	<input type='hidden' name='pagina_filha' value='movimentacao'>
	<input type='hidden' name='botao' value='1'>
...
$acha_compra = mysqli_query ($conexao, "SELECT * FROM compras WHERE estado_registro!='EXCLUIDO' AND movimentacao='COMPRA' 
AND cod_produto='$cod_produto' AND fornecedor='$fornecedor' AND filial='$filial' ORDER BY codigo");
...
for ($so=1 ; $so<=$linha_acha_compra ; $so++)
{
	...
	$soma_pgto = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(valor) FROM favorecidos_pgto WHERE estado_registro!='EXCLUIDO' 
	AND forma_pagamento!='PREVISAO' AND codigo_compra='$aux_acha_compra[1]'"));
