...
<div class="ct_topo_1">
	...
	<?php 
    if ($linha_compra == 1)
    {echo "$linha_compra Registro Exclu&iacute;do";}
    elseif ($linha_compra > 1)
    {echo "$linha_compra Registros Exclu&iacute;dos";}
    else
    {echo "";}
    ?>
    </div>
...
<!-- ================================================================================================================ -->
<div class="pqa_caixa">
<form action="<?php echo"$servidor/$diretorio_servidor"; ?>/compras/relatorios/relatorio_excluidos.php" method="post" />
...
<div class="pqa_caixa">
	...
	{echo "<option value='GERAL' selected='selected'>(TODAS)</option>";}
	else
	{echo "<option value='GERAL'>(TODAS)</option>";}

	if ($movimentacao_busca == "COMPRA")
	{echo "<option value='COMPRA' selected='selected'>Compras</option>";}
	else
	{echo "<option value='COMPRA'>Compras</option>";}

	if ($movimentacao_busca == "ENTRADA")
	{echo "<option value='ENTRADA' selected='selected'>Entradas</option>";}
	else
	{echo "<option value='ENTRADA'>Entradas</option>";}

	if ($movimentacao_busca == "SAIDA")
	{echo "<option value='SAIDA' selected='selected'>Sa&iacute;das</option>";}
	else
	{echo "<option value='SAIDA'>Sa&iacute;das</option>";}

	if ($movimentacao_busca == "TRANSFERENCIA")
	{echo "<option value='TRANSFERENCIA' selected='selected'>Transfer&ecirc;ncias</option>";}
	else
	{echo "<option value='TRANSFERENCIA'>Transfer&ecirc;ncias</option>";}
	?>
    </select>
    </div>
</div>
...
<div class="pqa_caixa">
    <div class="pqa_rotulo">
    Usu&aacute;rio Exclus&atilde;o:
    </div>
    
    <div class="pqa_campo">
    <select name="usuario_busca" class="pqa_select" style="width:190px" />
    <?php
	if ($usuario_busca == "GERAL")
	{echo "<option selected='selected' value='GERAL'>(TODOS)</option>";}
	else
	{echo "<option value='GERAL'>(TODOS)</option>";}

	include ("../../includes/conecta_bd.php");
	$busca_comprador = mysqli_query ($conexao,
	"SELECT
		username,
		primeiro_nome
	FROM
...
/*		WHERE		
	...
	for ($c=1 ; $c<=$linhas_comprador ; $c++)
	{
		...
		if ($aux_comprador[0] == $usuario_busca)
		{
...
<div style="width:auto; height:30px; border:1px solid transparent; margin-top:6px; float:left">
	...
	if ($linha_compra >= 1)
	{echo"
	<form action='$servidor/$diretorio_servidor/compras/relatorios/relatorio_excluidos_impressao.php' target='_blank' method='post' />
...
<div class='total' style='height:40px; width:384px; margin-top:0px' title=''>
	...
	<div class='total_valor' style='width:150px; height:15px; border:0px solid #999; font-size:12px; margin-top:13px'>$soma_quantidade_print</div>
</div>
</div>";
...
<td width='180px'>Movimenta&ccedil;&atilde;o</td>
<td width='220px'>Usu&aacute;rio Exclus&atilde;o</td>
<td width='100px'>Data Exclus&atilde;o</td>
</tr>
</table>";}
...
<td width='180px' align='center'>$movimentacao_w</td>
<td width='220px' align='left'><div style='height:14px; margin-left:10px; overflow:hidden'>$usuario_exclusao_w</div></td>
<td width='100px' align='center'>$data_exclusao_print</td>";
// =================================================================================================================
