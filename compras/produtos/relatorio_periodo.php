...
$busca_compra = mysqli_query ($conexao, "SELECT * FROM compras WHERE estado_registro!='EXCLUIDO' AND data_compra>='$data_inicial' AND data_compra<='$data_final' AND movimentacao='COMPRA' AND filial='$filial' ORDER BY codigo");
$linha_compra = mysqli_num_rows ($busca_compra);

$soma_compras = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(valor_total) FROM compras WHERE estado_registro!='EXCLUIDO' AND data_compra>='$data_inicial' AND data_compra<='$data_final' AND movimentacao='COMPRA' AND filial='$filial'"));
...
<div id="centro" style="height:35px; width:1080px; border:0px solid #000; margin:auto">
	...
	<div id="centro" style="height:30px; width:922px; border:1px solid #999; color:#666; font-size:11px; border-radius:5px; float:left">
		...
		<form action="<?php echo"$servidor/$diretorio_servidor"; ?>/compras/produtos/relatorio_periodo.php" method="post" />
...
<div id="centro" style="height:30px; width:1080px; border:0px solid #000; margin:auto">
	...
	if ($linha_compra >= 1)
	{echo"
	<form action='$servidor/$diretorio_servidor/compras/produtos/relatorio_periodo_impressao.php' target='_blank' method='post'>
