...
{
// ====== BUSCA E SOMA COMPRAS =================================================================================
$busca_compra = mysqli_query ($conexao, "SELECT * FROM compras WHERE estado_registro!='EXCLUIDO' AND data_compra>='$data_inicial' AND data_compra<='$data_final' AND movimentacao='COMPRA' AND fornecedor='$fornecedor' AND cod_produto='$cod_produto' AND filial='$filial' ORDER BY codigo");
$linha_compra = mysqli_num_rows ($busca_compra);
	
$soma_compras = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(valor_total) FROM compras WHERE estado_registro!='EXCLUIDO' AND data_compra>='$data_inicial' AND data_compra<='$data_final' AND movimentacao='COMPRA' AND fornecedor='$fornecedor' AND cod_produto='$cod_produto' AND filial='$filial'"));
$soma_compras_print = number_format($soma_compras[0],2,",",".");
// =======================================================================================================
}
...
<div style="width:1080px; height:30px; border:0px solid #000; margin:auto">
    ...
    Relat&oacute;rio de Compras - Por Fornecedor
    </div>
...
<div id="centro" style="height:30px; width:1080px; border:0px solid #000; margin:auto">
	...
	<?php 
		if ($fornecedor == "" or $linhas_fornecedor == 0)
		{echo"<div id='centro' style='height:25px; width:500px; border:0px solid #000; color:#F00; font-size:12px; float:left'> $fornecedor_print</div>";}
		else
		{echo"<div id='centro' style='height:25px; width:500px; border:0px solid #000; color:#00F; font-size:12px; float:left'><b> $fornecedor_print</b></div>";}
	?>
	<div id="centro" style="height:25px; width:70px; border:0px solid #000; color:#666; font-size:12px; float:left">
	<!-- Produto: -->
	</div>
	<div id="centro" style="height:25px; width:200px; border:0px solid #000; color:#00F; font-size:12px; float:left">
	<b><?php // echo"$produto_print"; ?></b>
	</div>
</div>
...
<div id="centro" style="height:35px; width:1080px; border:0px solid #000; margin:auto">
	...
	<div id="centro" style="height:30px; width:922px; border:1px solid #999; color:#666; font-size:11px; border-radius:5px; float:left">
		<form action="<?php echo"$servidor/$diretorio_servidor"; ?>/compras/produtos/relatorio_fornecedor.php" method="post" />
		...
		<select name="cod_produto" onkeydown="if (getKey(event) == 13) return false;" style="color:#0000FF; width:200px; height:21px; font-size:12px; text-align:left" />
...
<div id="centro" style="height:30px; width:1080px; border:0px solid #000; margin:auto">
	...
	if ($linha_compra >= 1)
	{echo"
	<form action='$servidor/$diretorio_servidor/compras/produtos/relatorio_fornecedor_impressao.php' target='_blank' method='post'>
...
$soma_compra_produto = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(valor_total) FROM compras WHERE estado_registro!='EXCLUIDO' AND data_compra>='$data_inicial' AND data_compra<='$data_final' AND movimentacao='COMPRA' AND fornecedor='$fornecedor' AND cod_produto='$aux_bp_geral[0]' AND filial='$filial'"));
$soma_cp_print = number_format($soma_compra_produto[0],2,",",".");
$soma_quant_produto = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(quantidade) FROM compras WHERE estado_registro!='EXCLUIDO' AND data_compra>='$data_inicial' AND data_compra<='$data_final' AND movimentacao='COMPRA' AND fornecedor='$fornecedor' AND cod_produto='$aux_bp_geral[0]' AND filial='$filial'"));
