...
$busca_compra = mysqli_query ($conexao, "SELECT * FROM compras WHERE estado_registro!='EXCLUIDO' AND data_compra='$data_inicial' AND movimentacao='COMPRA' AND filial='$filial' ORDER BY codigo");
$linha_compra = mysqli_num_rows ($busca_compra);

$soma_compras = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(valor_total) FROM compras WHERE estado_registro!='EXCLUIDO' AND data_compra='$data_inicial' AND movimentacao='COMPRA' AND filial='$filial'"));
...
<div id="centro" style="height:35px; width:1080px; border:0px solid #000; margin:auto">
	...
	<div id="centro" style="height:30px; width:922px; border:1px solid #999; color:#666; font-size:11px; border-radius:5px; float:left">
		<div id="centro" style="width:50px; float:left; height:20px; color:#666; border:0px solid #999; text-align:right">
		<div id="geral" style="width:45px; height:8px; float:left; border:0px solid #999"></div>
		<form action="<?php echo"$servidor/$diretorio_servidor"; ?>/compras/produtos/relatorio_data.php" method="post" />
...
<div id="centro" style="height:30px; width:1080px; border:0px solid #000; margin:auto">
	...
	if ($linha_compra >= 1)
	{echo"
	<form action='$servidor/$diretorio_servidor/compras/produtos/relatorio_data_impressao.php' target='_blank' method='post'>
...
$soma_compra_produto = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(valor_total) FROM compras WHERE estado_registro!='EXCLUIDO' AND data_compra='$data_inicial' AND movimentacao='COMPRA' AND cod_produto='$aux_bp_geral[0]' AND filial='$filial'"));
$soma_cp_print = number_format($soma_compra_produto[0],2,",",".");
$soma_quant_produto = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(quantidade) FROM compras WHERE estado_registro!='EXCLUIDO' AND data_compra='$data_inicial' AND movimentacao='COMPRA' AND cod_produto='$aux_bp_geral[0]' AND filial='$filial'"));
