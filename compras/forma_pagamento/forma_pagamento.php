...
$idContratoSankhya = $_POST["pedidoSankhya"] ?? '';
...
$valor_pagamento = Helpers::ConverteValor($_POST["valor_pagamento"] ?? 0);
$banco_ted = '';
$agencia_ted_aux = '';
$tipo_conta = '';
...
<body onload="loading()">
	...
	// Mensagem do loading
	$loadTitle = 'Aguarde...';
	$loadMessage = 'Processando forma de pagamento dos favorecidos.';
	...
	try {
		...
		for ($f = 1; $f <= $linha_acha_favorecido; $f++) {
			...
			$idFavorecidoSankhya = $aux_fav[23];
			$favorecido_rovereti = Helpers::TiraAcentos($nome_fav);
		...
		if ($forma_pagamento == "PREVISAO") {
			...
			for ($e = 1; $e <= $linha_acha_favorecido_2; $e++) {
				...
				$idFavorecidoSankhya =  $aux_fav_2[23];
			}
		}
		...
		if ($forma_pagamento == "PREVISAO") {
			...
			// Valida informações para o Sankhya
			if (!$erro) {
				// Busca produto para obter a quantidade da unidade
				$sql = "select quantidade_un, id_sankhya 
		          from cadastro_produto
				 where codigo = $cod_produto";

				$resultProduto = Sankhya::queryExecuteDB($sql);
				$unidadeProduto = $resultProduto['rows'][0][0];
				$idProdutoSankhya = $resultProduto['rows'][0][1];

				if (!$idProdutoSankhya) {
					$errorSankhya = 1;
					$msgSankhya   = 'Código Sankhya do produto não informado no SUIF.';
				} elseif (!$unidadeProduto) {
					$errorSankhya = 2;
					$msgSankhya   = 'Quantidade da unidade do produto não informado no SUIF.';
				}
				...
				$resultProdutor = Sankhya::queryExecuteDB($sql);
				$produtorSankhya = $resultProdutor['rows'][0][0];

				if (!$produtorSankhya) {
					$errorSankhya = 3;
					$msgSankhya   = 'Código Sankhya do produtor não informado.';
				}
				...
				$sql = "select b.id_sankhya, a.sequencia_cc_sankhya
			 	          from cadastro_favorecido a,
		 		           ...
		 		           and b.codigo_pessoa = a.codigo_pessoa";
			...
			if (!$erro && !$errorSankhya) {
				...
				$idFavorecidoPgto = mysqli_insert_id($conexao);

				include("../../includes/desconecta_bd.php");


				include("../../includes/conecta_bd.php");
				$editar_compra = mysqli_query(
					$conexao,
					"UPDATE
							compras
						SET
				...
				$resultFaturamento = Sankhya::faturaPedidoCompra($numero_compra, $qtdeFaturar, $data_pagamento_print, $valor_pagamento);

				if ($resultFaturamento['errorCode']) {
					$errorSankhya = $resultFaturamento['errorCode'];
					$msgSankhya   = $resultFaturamento['errorMessage'];
				} else {
					$pedidoFaturamento  = $resultFaturamento['rows']['notas']['nota']['$'];
				}

				if (!$errorSankhya && $pedidoFaturamento) {
					$resultAlteraItem = Sankhya::alteraItemNota(
						$pedidoFaturamento,
						$idProdutoSankhya,
						$valor_pagamento,
						$qtdeFaturar,
					...
					if ($resultAlteraItem['errorCode']) {
						$errorSankhya = $resultAlteraItem['errorCode'];
						$msgSankhya   = $resultAlteraItem['errorMessage'];
					}
				}
				...
				if (!$errorSankhya && $pedidoFaturamento) {
					...
					if ($resultCabecalhoNF['errorCode']) {
						...
						// Confirma pedido de faturamento
						$resultConfirma = Sankhya::confirmaPedidoCompra($pedidoFaturamento);

						if ($resultConfirma['errorCode']) {
							$errorSankhya = $resultConfirma['errorCode'];
							$msgSankhya   = $resultConfirma['errorMessage'];
						} else {
							$msgSankhya = "Fatura $pedidoFaturamento gerada no Sankhya.";
						}

						$pedidoConfirmado = $resultConfirma['errorCode'] ? 'N' : 'S';
					}
				}

				if ($idFavorecidoPgto) {
					$sql = "update favorecidos_pgto 
					set id_pedido_sankhya = $pedidoFaturamento,
						pedido_confirmado_sankhya = '$pedidoConfirmado'
					where codigo = $idFavorecidoPgto";

					$resultConfirma = Sankhya::queryExecuteDB($sql);
				}
			...
			include("../../includes/conecta_bd.php");
			$excluir = mysqli_query($conexao, "UPDATE favorecidos_pgto SET estado_registro='EXCLUIDO', usuario_exclusao='$usuario_alteracao', data_exclusao='$data_alteracao', hora_exclusao='$hora_alteracao' WHERE codigo='$codigo_pgto_favorecido'");
			include("../../includes/desconecta_bd.php");
	...
	<div id="centro_geral">
		...
		<div id="centro" style="height:390px; width:1080px; border:0px solid #0000FF; margin:auto">
			...
			<form name="forma_pagamento" action="<?php echo "$servidor/$diretorio_servidor"; ?>/compras/forma_pagamento/forma_pagamento.php" method="post" onsubmit="showLoading()">
				...
				<div id="tabela_2" style="width:130px; height:19px; border:0px solid #000">
					<div id="espaco_1" style="width:158px; height:5px; border:0px solid #000"></div>Cheque Banco:
				</div>

				<div id="tabela_1" style="width:30px; height:19px; border:0px solid #000"></div>
				<div id="tabela_2" style="width:90px; height:19px; border:0px solid #000">
					<div id="espaco_1" style="width:88px; height:5px; border:0px solid #000"></div>N&ordm; Cheque:
				</div>
				...
				<div id="tabela_2" style="width:620px; border:0px solid #000; float:left">
					<div id="centro" style="float:left; border:0px solid #000; margin-top:0px; font-size:12px">
						...
						<div id="centro" style="float:left; border:0px solid #000; margin-top:3px">
							<img src="<?php echo "$servidor/$diretorio_servidor"; ?>/imagens/icones/buscar.png" style="border: 0px; height: 18px;" onclick="javascript:abrir('busca_pessoa_popup.php'); javascript:foco('busca');" title="Pesquisar produtor" />
							<a href="<?php echo "$servidor/$diretorio_servidor"; ?>/cadastros/pessoas/cadastro_1_formulario.php" target="_blank">
								<img src="<?php echo "$servidor/$diretorio_servidor"; ?>/imagens/icones/editar.png" style="border: 0px; height: 18px" title="Cadastrar novo fornecedor" /></a>
						</div>
	...
	<div id="centro" style="height:auto; width:1050px; border:1px solid #999; margin:auto; border-radius:5px;">
		...
		if ($linha_favorecidos_pgto == 0) {
			...
			echo "
					...
					<td width='240px' align='center' bgcolor='#006699'>Favorecido</td>
					<td width='100px' align='center' bgcolor='#006699'>Forma Pgto</td>
					<td width='180px' align='center' bgcolor='#006699'>Dados Banc&aacute;rios</td>
					<td width='100px' align='center' bgcolor='#006699'>Quantidade Ref.</td>
					<td width='70px' align='center' bgcolor='#006699'>Valor (R$)</td>
					<td width='70px' align='center' bgcolor='#006699'>Sankhya</td>
		...
		for ($w = 1; $w <= $linha_favorecidos_pgto; $w++) {
			...
			$idPedidoSankhyaConfirmado = $aux_favorecido[39];
			...
			if ($conta_conjunta == "SIM") {
				echo "<td width='240px' align='left'>&#160;&#160;$nome_favorecido_2 - $aux_favorecido[2] (*)</td>";
			} else {
				echo "<td width='240px' align='left'>&#160;&#160;$nome_favorecido_2 - $aux_favorecido[2]</td>";
			}
			...
			echo "
					...
					<td width='180px' align='left'>&#160;&#160;$dados_bancarios_2</td>
					<td width='100px' align='center'>$quant_ref_print $unidade</td>
					<td width='70px' align='right'>$valor_pagamento_print_2&#160;&#160;</td>
					<td width='70px' align='center'>$idPedidoSankhya</td>";
	...
	<div id="centro" style="height:180px; width:1030px; border:0px solid #999; margin:auto; border-radius:5px; text-align:center">
		...
		<div style="float:left; width:1030px; height:auto; border:0px solid #000; font-size:10px; color:#999; text-align:center">
			...
			<div class="badge-success" role="alert" <?= $errorSankhya ? 'hidden' : '' ?>>
				<?= $msgSankhya ?>
			</div>

			<div class="badge-danger" role="alert" <?= !$errorSankhya ? 'hidden' : '' ?>>
				<?= $msgSankhya ?>
			</div>
