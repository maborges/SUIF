...
$botao = $_POST["botao"];
	...
	$busca_pagamento = mysqli_query ($conexao, "SELECT * FROM favorecidos_pgto WHERE estado_registro!='EXCLUIDO' AND data_pagamento>='$data_inicial' AND data_pagamento<='$data_final' AND codigo_fornecedor='$fornecedor' AND filial='$filial' ORDER BY codigo");
	$linha_pagamento = mysqli_num_rows ($busca_pagamento);


	// SOMA PAGAMENTOS  ==========================================================================================
	$soma_pagamentos = mysqli_fetch_row(mysqli_query ($conexao, "SELECT SUM(valor) FROM favorecidos_pgto WHERE estado_registro!='EXCLUIDO' AND data_pagamento>='$data_inicial' AND data_pagamento<='$data_final' AND codigo_fornecedor='$fornecedor' AND filial='$filial' AND situacao_pagamento='PAGO'"));
...
<div id="centro" style="height:35px; width:1080px; border:0px solid #000; margin:auto">
	...
	&#160;&#160;&#8226; <b>Relat&oacute;rio de Pagamentos</b> - Por Preposto
	</div>
...
<div id="centro" style="height:35px; width:1080px; border:0px solid #000; margin:auto">
	...
	<div id="centro" style="height:30px; width:1060px; border:1px solid #999; color:#666; font-size:11px; border-radius:5px; float:left">
		...
		<form name="compra_cafe" action="<?php echo"$servidor/$diretorio_servidor"; ?>/financeiro/compras/relatorio_pagamentos.php" method="post" />
