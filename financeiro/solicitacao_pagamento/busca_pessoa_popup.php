...
<form action="<?php echo"$servidor/$diretorio_servidor"; ?>/financeiro/solicitacao_pagamento/busca_pessoa_popup.php" method="post" />
...
for ($x=1 ; $x<=$linha_pessoa ; $x++)
{
	$aux_pessoa = mysqli_fetch_row($busca_pessoa);
		...
		{$cpf_cnpj_print = $aux_pessoa[3];}
		else
		{$cpf_cnpj_print = $aux_pessoa[4];}


// === BUSCA FAVORECIDO ===================================================================================================	
	$busca_favorecido = mysqli_query ($conexao, "SELECT * FROM cadastro_favorecido WHERE estado_registro='ATIVO' AND codigo_pessoa='$aux_pessoa[35]' ORDER BY codigo");
	$linha_favorecido = mysqli_num_rows ($busca_favorecido);
	
	for ($f=1 ; $f<=$linha_favorecido ; $f++)
	{
	$aux_favorecido = mysqli_fetch_row($busca_favorecido);

// === BUSCA BANCO ========================================================================================================	
		$busca_banco = mysqli_query ($conexao, "SELECT * FROM cadastro_banco WHERE numero='$aux_favorecido[2]' ORDER BY codigo");
		$linha_banco = mysqli_num_rows ($busca_banco);
		$aux_banco = mysqli_fetch_row($busca_banco);



		if ($aux_favorecido[5] == 'corrente' and $aux_favorecido[4] != '')
		{$dados_bancarios = '&#160;&#160;(' . $aux_banco[3] . ' ) &#160;&#160;AG:' . $aux_favorecido[3] . ' &#160;&#160;C/C: ' . $aux_favorecido[4];}
		
		elseif ($aux_favorecido[5] == 'poupanca' and $aux_favorecido[4] != '')
		{$dados_bancarios = '&#160;&#160;(' . $aux_banco[3] . ' ) &#160;&#160;AG:' . $aux_favorecido[3] . ' &#160;&#160;C/P: ' . $aux_favorecido[4] . '&#160;&#160;(Poupan&ccedil;a)';}

		elseif ($aux_favorecido[5] == 'salario' and $aux_favorecido[4] != '')
		{$dados_bancarios = '&#160;&#160;(' . $aux_banco[3] . ' ) &#160;&#160;AG:' . $aux_favorecido[3] . ' &#160;&#160;C/A: ' . $aux_favorecido[4];}
		
		elseif ($aux_favorecido[5] == 'aplicacao' and $aux_favorecido[4] != '')
		{$dados_bancarios = '&#160;&#160;(' . $aux_banco[3] . ' ) &#160;&#160;AG:' . $aux_favorecido[3] . ' &#160;&#160;C/A: ' . $aux_favorecido[4];}
...
// ==========================================================================================================	
		...
		else
		{
			...
			<td width='640px' align='left' style='title='CPF/CNPJ: $cpf_cnpj_print'>
			<a href=javascript:retorna('$aux_favorecido[0]'); style='text-decoration:none; color:#00F'>&#160;&#160;$aux_favorecido[0] - $aux_pessoa[1] $dados_bancarios</a></td>";
